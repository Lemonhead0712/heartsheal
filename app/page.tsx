...
// ── Daily healing quotes (rotate by day-of-year) ─────────────────────────────
const HEALING_QUOTES = [
  { text: "Healing doesn't mean the damage never existed. It means it no longer controls your life.", author: "Akshay Dubey" },
  ...
  { text: "Grief is the price we pay for love — and it is worth every penny.", author: "Unknown" },
  { text: "What you carry does not define you. How you tend to it does.", author: "Haven" },
]
...
export default function HavenHome() {
  ...
  // Derived: show welcome whenever auth is resolved and there's no signed-in user
  // guestContinued lets a user dismiss it for the session without signing in
  const [guestContinued, setGuestContinued] = useState(false)
  const welcomeOpen = !authLoading && !user && !guestContinued
  const welcomeOpenRef = useRef(false)   // sync ref so audio guards don't need re-render
  const [onboardingOpen, setOnboardingOpen] = useState(false)
  const [authModalOpen, setAuthModalOpen] = useState(false)
  const [authModalMode, setAuthModalMode] = useState<"signin" | "signup">("signup")
  const welcomeShownRef = useRef(false)
  // Keep a stable ref to the current user so callbacks always read the latest value
  ...
  // Keep welcomeOpenRef in sync + mute audio while welcome overlay is up
  // (No deps array — runs every render to keep ref always accurate)
  useEffect(() => {
  ...
  // Trigger onboarding once after the user signs in through the welcome screen
  useEffect(() => {
    if (!authLoading && user && welcomeShownRef.current && !readStorage(STORAGE_KEYS.welcomeSeen)) {
  ...
  const handleEmotionPick = useCallback(async (label: string, emoji: string, intensity: number) => {
    if (!userRef.current) { setAuthModalMode("signup"); setAuthModalOpen(true); return }
    ...
    <div className="flex flex-col bg-background h-[calc(100dvh-140px)] md:h-auto md:flex-1">
      ...
      <header className="md:hidden flex items-center justify-between px-4 pt-2 pb-1 shrink-0">
        <div className="flex items-center gap-1.5 text-muted-foreground/60 text-sm">
          ...
          <span className="font-serif font-semibold text-foreground/70 tracking-tight">Haven</span>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center px-4 pt-4 min-h-0 overflow-y-auto">

        {/* Top spacer — widgets need a little room */}
        ...
        {/* chips rendered in the bottom Haven bar */}
        ...
        {!loading && (mode === "greeting" || mode === "chatting") && (
          <motion.div
            ...
            className="w-full max-w-lg mt-4 pb-4"
          >
            ...
            {(() => {
              ...
              return (
                <div className="rounded-2xl p-4 mb-4 bg-gradient-to-br from-primary/8 to-primary/4 border border-primary/15">
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-primary/50 mb-2">Today's reflection</p>
                  <p className="text-sm font-serif italic text-foreground/80 leading-relaxed mb-1.5">"{q.text}"</p>
                  <p className="text-[11px] text-muted-foreground/60 text-right">— {q.author}</p>
                </div>
              )
            })()}

            {/* What to do next guidance (only before emotion logged) */}
            {!completedToday.has("emotion") && (
              <div className="rounded-2xl p-4 mb-4 border border-border/40 bg-card/60">
                <p className="text-xs font-semibold text-foreground mb-1.5">Where to begin</p>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Start by logging how you're feeling. Haven will build a personalised healing session — breathing, journaling, reflection, and a wellbeing check — guided by your emotion.
                </p>
              </div>
            )}
      ...
      {/* ── Haven bar — styled exactly like HavenFlowGuide bottom nav ── */}
      <div className="shrink-0 bg-background/92 backdrop-blur-xl border-t border-border/40 shadow-2xl shadow-black/10">
        <div className="w-full max-w-lg mx-auto px-4 py-3 space-y-2.5">

          {/* Row 1 — Haven label + voice + streak */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-muted-foreground/60">
              <Sparkles className="w-3.5 h-3.5 text-primary/70 shrink-0" />
              <span className="text-xs font-semibold text-primary/70">Haven</span>
            </div>
            <div className="flex items-center gap-2">
              {streak > 0 && (
                <span className="text-[10px] font-semibold text-primary/60">🔥 {streak}</span>
              )}
              <button
                onClick={voiceEnabled ? stopSpeech : toggleVoice}
          ...
          {/* Row 2 — Haven orb + typewriter message (identical to HavenFlowGuide) */}
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  {loading ? (
                    ...
                    <p className="text-sm text-foreground/85 font-serif leading-snug">{displayText}</p>
                  )}
                </motion.div>
              </AnimatePresence>
            </div>
          ...
          {/* Row 3 — chips (only in greeting/chatting, no widget open) */}
          {!loading && chips.length > 0 && !widgetActive && (
            <div className="flex flex-wrap gap-2">
              {chips.map((chip) => (
                <button
                  key={chip}
                  onClick={() => sendToHaven(chip)}
                  className="px-3.5 py-1.5 rounded-full border border-border/50 bg-card/60 text-xs font-medium text-foreground hover:border-primary/50 hover:bg-primary/5 hover:text-primary transition-all active:scale-95"
                >
                  {chip}
                </button>
              ))}
